...
-------------------------------------------------------------------------------------------------------------------------------
 ...
 Revision History:
    ...
    2025-09-27 - jpl    Changed code from == to =. in getString_cfgfileParameter line 294 to fix a warning, cleaned up
...
int cfgfileHandler::getString_cfgfileParameter(char *key, int parmNo, char **retStr) {
	...
	//   out and process the parameter
		for(i = 0; i < cfgfileNoRcrds; i++) {
			...
			if(retPtr != nullptr) {indx = i; cvtRec = 1; break;}			
		}
...
int cfgfileHandler::getDouble_cfgfileParameter(char *key, int parmNo, double *retDouble) {
